...
- Name: shell-navigation.spec.ts >> Shell App — Cloud Navigation >> sidebar navigation links are present
- Location: e2e-cloud\shell-navigation.spec.ts:10:7
...
Locator: locator('nav, [class*="sidebar"], [class*="Sidebar"]').first().getByText('Dashboard')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('nav, [class*="sidebar"], [class*="Sidebar"]').first().getByText('Dashboard')
