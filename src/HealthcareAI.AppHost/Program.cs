...
// Microservices
// ──────────────────────────────────────────────
var identityService = builder.AddProject<Projects.HealthcareAI_Identity>("identity-service")
    .WithReference(identityDb)
    .WithReference(redis)
    .WithExternalHttpEndpoints();

var voiceService = builder.AddProject<Projects.HealthcareAI_Voice>("voice-service")
    .WithReference(voiceDb)
    .WithReference(redis)
    .WithExternalHttpEndpoints();

var agentService = builder.AddProject<Projects.HealthcareAI_Agents>("agent-service")
    .WithReference(agentDb)
    .WithReference(redis)
    .WithExternalHttpEndpoints();

var fhirService = builder.AddProject<Projects.HealthcareAI_Fhir>("fhir-service")
    .WithReference(fhirDb)
    .WithReference(redis)
    .WithExternalHttpEndpoints();

var ocrService = builder.AddProject<Projects.HealthcareAI_Ocr>("ocr-service")
    .WithReference(ocrDb)
    .WithReference(redis)
    .WithExternalHttpEndpoints();

var schedulingService = builder.AddProject<Projects.HealthcareAI_Scheduling>("scheduling-service")
    .WithReference(schedulingDb)
    .WithReference(redis)
    .WithExternalHttpEndpoints();

var notificationService = builder.AddProject<Projects.HealthcareAI_Notifications>("notification-service")
    .WithReference(notificationDb)
    .WithReference(redis)
    .WithExternalHttpEndpoints();

var pophealthService = builder.AddProject<Projects.HealthcareAI_PopulationHealth>("pophealth-service")
